...
var Tool = {
    ...
    , Step(...stepArg) {
        var onearg = stepArg[0]
            , len = stepArg.length
            , wrongfunc = function (e) {
                console.log(e)
            }
    ...
    , union<T, U>(first: T, second: U): T & U {
        let result = <T & U>{};
        for (let id in first) {
    ...
    , initChecker() { //put "how to deil with Checker behind" and help you more smoothly coding
        var checkStore = {}
    ...
    , initErrorManager() {
        ...
        var Arg = {
            add(name, status) {
                Tool.set(errorStore, name, []).push({ //same place may occurse many error;
                    status
                    , message: "wrong!"
                })
    ...
    , events: {
        ...
        , initManager(fromDoc) {
            ...
            var self = {
                ...
                , eventFuncStore: null
                , analys(x) { //解析事件名
                    if (self.eventStore[x]) return self.eventStore[x]
    ...
    , dom: {
        initPicker(dom) {
            ...
            var Arg = {
                ...
                , get(tagstr, option?, forEachfunc?) {
                    if (tagstr == undefined) return Arg.dom;
        ...
        , initSuper() {
            var doc = document.getElementById("") //let IED know that doc is an Element;
                ...
                , pk = Tool.dom.initPicker(document)
                , all = {}
            for (let x in func) {
                all[x] = function (...args) {
    ...
    , struct: {
        ...
        initSet(set?) {
            ...
            var Arg = {
                get(str) {
                    if (str == undefined) return Object.keys(set);
                    return set[str]
                }
                , add(str) {
                    if (set[str]) {
    ...
    , str: {
        ...
        , treeText(str) {
            ...
            function addline() {
                ...
                if (onepl.length == 0) return true

                text.lines.push(onepl)
                onepl = []
            }
            function addword() {
                if (onepw == '') return true
    ...
    , initFunc(usage: string, op, ext?) {
        ...
        if (typeof ext == "object") {
            ...
            if (typeof op.args == "string") op.args = JSON.parse(op.args);
        }
        if (!op.args) {
            op.args = {}
        }
    ...
    , math: {
        getRandom(arr) { //get random one from the array;
            var len = arr.length;
...
var hv = {
    addSetter(obj: object, key: string, Origin_gt: getsetRus, From_gt: getsetRus, fromObj) {
        var check = true;
        function get() {
    ...
    , redefine(gt: getset & getsetExt, From_gt, fromObj) {
        var Origin_gt = {
            get: gt.get
            , set: gt.set
        }
